...
collector = load_module("ascend_collect_cases", "collect_cases.py")
accuracy_runner = load_module("ascend_run_accuracy", "run_accuracy.py")
native_runner = load_module("ascend_run_native", "run_native.py")
longtail_runner = load_module("ascend_run_longtail", "run_longtail.py")
transformer_runner = load_module("ascend_run_transformer", "run_transformer_block.py")
...
class CollectorTests(unittest.TestCase):
    ...
    def test_gemm_reads_current_ascend_baseline(self):
        fields = [
    ...
    def test_conv_requires_cuda_style_component_sum(self):
        fields = [
        ...
        bad = conv_rows()
        bad[0]["baseline"] = 0.7
        write_csv(self.root / "conv" / "conv_f32.csv", fields, bad)
        with self.assertRaisesRegex(ValueError, "forward"):
            collector.build_result(self.root, 2.0, "conv")
    ...
    def test_accuracy_collects_each_available_dtype(self):
        ...
        (directory / "npu_val_result.json").write_text(
...
class RunnerHelperTests(unittest.TestCase):
    ...
    def test_native_input_requires_empty_measurements(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            source = root / "gemm.csv"
            output = root / "gemm_input.csv"
            fields = list(native_runner.CONFIGURATIONS["gemm"]["columns"])
            rows = gemm_rows(1.0)
            for row in rows:
                row["baseline"] = ""
            write_csv(source, fields, rows)
            identities = native_runner.prepare_input(source, output, "gemm")
            self.assertEqual(len(identities), 224)
            rows[0]["baseline"] = 1.0
            write_csv(source, fields, rows)
            with self.assertRaisesRegex(ValueError, "baseline must be empty"):
                native_runner.prepare_input(source, output, "gemm")
