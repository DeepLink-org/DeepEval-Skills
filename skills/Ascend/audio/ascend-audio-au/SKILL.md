---
name: ascend-audio-au
description: Ascend NPU 上语音理解模型推理性能评测技能。支持 lang-id-voxlingua107-ecapa 语种识别模型，用于指导 executor 完成容器启动、数据集准备、推理执行、日志采集与性能指标分析（ACC）。
---

### 触发条件

当用户说以下任意内容时启动：
- "我要在 ascend 上跑 AU 模型测试"
- "帮我在昇腾 NPU 上测试 lang-id-voxlingua107-ecapa 语种识别模型推理性能"
- "在 ascend 上跑语音理解 foundation-lid 数据集测试集"
...
**Docker 镜像**：
```bash
swr.cn-north-1.myhuaweicloud.com/deeplink/ascend-audio-au:latest
```
...
| AU 框架 | `speechbrain` | 1.1.0 | ECAPA-TDNN 语种识别推理框架 |
...
```text
结果: 1000/1000 成功
指标: {'acc': 0.653, 'correct': 653, 'num_samples': 1000}
耗时: 40.7s
```
...
============================================================
评测数据集数: 1
  foundation-lid: {'acc': 0.653, 'correct': 653, 'num_samples': 1000}, 样本数=1000
  ...
  "records": [
    {
      ...
      "error_samples": 0,
      "success_rate": 1.0,
      "total_time": 66.62904405593872,
      "avg_inference_time": 0.035263131141662594,
      "metrics": {
        "accuracy": 0.652,
        "num_samples": 1000
      },
      "timestamp": "20260615_022440"
    }
  ],
  "last_updated": "20260615_022440",
  "notes": ""
}
```
