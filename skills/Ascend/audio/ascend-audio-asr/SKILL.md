...
**Docker 镜像**：
```bash
swr.cn-north-1.myhuaweicloud.com/deeplink/ascend-audio-asr:latest
```
...
1. **Docker 容器启动失败**
   ...
   - **镜像未拉取**：确认镜像 `swr.cn-north-1.myhuaweicloud.com/deeplink/ascend-audio-asr:latest` 已 pull 到本地
