#!/usr/bin/env python3
"""Convert MetaX operator artifacts to AIBench Result Contract 2.0."""
...
def _read_gemm(source: Path, dtype: str) -> List[Case]:
    # The benchmark scripts may rewrite the CSV and omit empty time/score
    # columns.  Result Contract 2.0 only needs the measured baseline; when
    # optional columns are present they are still required to remain empty.
    required = {"NO", "M", "N", "K", "transA", "transB", "baseline"}
    cases: List[Case] = []
    seen: set[tuple] = set()
...
def _read_longtail(source: Path, dtype: str, run_token: str) -> Tuple[List[Case], int]:
    # LongTail-Bench can omit empty time/score columns in its rewritten CSV;
    # only baseline and the run token are required for the contract.
...
def _load_cases(
    ...
    if benchmark == "accuracy":
        relative = Path("accuracy/mx_val_result.json")
    ...
    if benchmark in {"gemm", "conv"}:
        # Match NVIDIA collector behavior: GEMM/Conv artifacts do not require
        # a separate fingerprint file.  The active workload fingerprint is
        # recorded in result metadata from the environment below.
    ...
    if benchmark == "longtail":
        ...
        for filename, dtype in (("longtail_fp32.csv", "f32"), ("longtail_fp16.csv", "f16")):
            ...
            if file_attempted < 1:
                raise ValueError(f"{source}: no LongTail cases found")
            identities = _identities_without_dtype(file_cases)
            if reference_identities is None:
                reference_identities = identities
            elif dtype == "f32" and identities != reference_identities:
                raise ValueError("LongTail fp32 case identities differ")
            elif dtype == "f16" and not set(identities).issubset(set(reference_identities)):
                raise ValueError("LongTail fp16 contains cases absent from fp32")
...
def build_result(
    input_dir: Path, duration_seconds: float, benchmark: str = "gemm"
) -> Dict[str, object]:
