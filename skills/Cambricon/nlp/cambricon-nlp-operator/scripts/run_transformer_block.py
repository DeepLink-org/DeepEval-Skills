#!/usr/bin/env python3
"""Deterministically benchmark MLU Transformer encoder/decoder inference."""
...
def _measure_latency_ms(
    ...
    if not math.isfinite(latency_ms) or latency_ms <= 0:
        raise RuntimeError("measured latency must be a positive finite number")
    return latency_ms
...
def run_benchmark(
    ...
    *,
    ...
    if not torch.mlu.is_available():
        raise RuntimeError("MLU is required for Transformer Block benchmark")
    ...
    torch.manual_seed(0)
    if hasattr(torch.mlu, "manual_seed_all"):
        torch.mlu.manual_seed_all(0)

    encoder = EncoderLayer(d_model, ffn_hidden_size, num_heads, drop_prob=0.1).to(device)
    encoder.eval()
    encoder_input = torch.rand(
        batch_size, sequence_length, d_model, device=device, dtype=torch.float32
    )
    encoder_latency = _measure_latency_ms(
        lambda: encoder(encoder_input, None), warmup_iterations, measurement_iterations
    )
    del encoder, encoder_input
    if hasattr(torch.mlu, "empty_cache"):
        torch.mlu.empty_cache()

    decoder = DecoderLayer(d_model, ffn_hidden_size, num_heads, drop_prob=0.1).to(device)
    decoder.eval()
    target = torch.rand(
        batch_size, sequence_length, d_model, device=device, dtype=torch.float32
    )
    ...
    common: Dict[str, object] = {
        ...
        "aibench_workload_fingerprint": _required_env(
...
def main() -> int:
    ...
    parser.add_argument(
        "--project-root",
        type=Path,
        default=Path("/workspace/operators/speed_test/transformer_block"),
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("/workspace/results/transformer/transformer_block_cases.csv"),
    )
