...
description: "nvidia芯片-CV场景-预训练/分类任务的评测流程。用于指导executor在 nvidia 环境下完成 onedl-mmpretrain 分类模型训练环境准备、批量脚本执行、性能采集和结果分析。适用于 resnet50、inceptionv3、seresnet50、mobilenetv2、shufflenetv2、densenet121、swin-large、efficientnet-b2 等模型。"
---
...
**日志路径**：
```
./models/onedl-mmpretrain/work_dirs/<MODEL_NAME>_gpus<NGPU>_<PRECISION>/<TIMESTAMP>/<TIMESTAMP>.log
```
...
5. **数据集报错或找不到数据**
   - 检查数据 ImageNet 目录或软链接是否按 `mmpretrain` 要求准备完成
