...
- 脚本会自动查找 mmdetection 源码目录；数据集路径由镜像内 `configs/_base_/datasets/coco_detection.py` 直接指向 `/workspace/datasets/coco`
...
4. **数据集路径错误**：检查 `CV_DET_DATA_DIR=/workspace/datasets/coco` 是否包含 COCO 数据集；当前镜像内 `configs/_base_/datasets/coco_detection.py` 应直接从该路径读取。
