...
# Resolve task inputs through the container mount points. Host-side run_dir is
# intentionally not used because only these mounts are visible in the
# container.
...
resolve_dataset_path() {
  ...
  for dataset_path in \
    "$DATASET_ROOT/llama2_7b_sharegpt.json" \
    "$DATASET_ROOT/ShareGPT_V3_unfiltered_cleaned_split.json"; do
