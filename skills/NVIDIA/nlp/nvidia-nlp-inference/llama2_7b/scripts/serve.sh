...
case "$PRECISION" in
  fp16)
    ...
    ;;
  int8)
    # The benchmark image exposes TorchAO INT8 weight-only quantization.
    # This quantizes the loaded FP16 checkpoint at startup, so no separate
    # pre-quantized model directory is required.
    precision_args=(--dtype float16 --torchao-config int8wo)
    ;;
  *)
    echo "PRECISION must be one of: fp16, int8 (got: $PRECISION)" >&2
    exit 2
    ;;
esac
