...
description: "NVIDIA GPU 上基于 sglang 的 DeepSeek 文本推理评测技能。用于指导 executor 完成模型服务启动、压测脚本执行、日志采集和性能结果分析。"
---
...
**硬件要求**：
- 8 张 NVIDIA GPU（当前 `serve.sh` 中 `--tp 8`）
...
docker run -d --gpus all --shm-size=100g \
  ...
  sglang:nightly-dev-20251208-5e2cda61
```
