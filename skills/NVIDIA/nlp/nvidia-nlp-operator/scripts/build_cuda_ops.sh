...
cmake -S "$CUDA_OPS_DIR" -B "$CUDA_OPS_DIR/build" 2>&1 | tee "$OPERATOR_LOGS_DIR/compile.log"
cmake --build "$CUDA_OPS_DIR/build" -j"${BUILD_JOBS:-$(nproc)}" 2>&1 | tee -a "$OPERATOR_LOGS_DIR/compile.log"
